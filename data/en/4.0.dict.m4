...
% [pv+].4: this gives Pv+ a fractional cost, so that Pa+ is preferred.
%     for example: "the player is injured" should get Pa+ not Pv+.
<vc-be-no-obj>:
...
% Cost on Omm- so that EA+ is prefered.
more:
