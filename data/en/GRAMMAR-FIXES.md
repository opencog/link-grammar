...
| --- | --- | --- |
| 1-2 | `SI#*` / `SI#x` embedded and fronted inversion | Simple removal raises `corpus-basic.batch` errors by accepting embedded and fronted inversion negatives. Rule 1 removal accepts eleven starred examples, including `*I know how quickly did you run` and `*I wonder how much money have you earned`; it also improves `corpus-fixes.batch` by one and `corpus-failures.batch` by three, so the rule is mixed rather than purely obsolete. Rule 2 removal accepts starred examples such as `*After the movie did he realize his mistake` and `*I wonder which dog did he say you chased`. A first dictionary-certificate prototype also failed: requiring non-empty auxiliary evidence is too broad because bad embedded cases can still have real `Rw` links, while a single direct-question certificate does not cover valid `SIpx` and extraction cases such as `Which dog did he say you chased?`, `Why did you leave?`, and `Where did you put it?`. A replacement needs evidence that the `SI` link is licensed by the same matrix/fronted/question domain, not merely by a locally question-shaped auxiliary connector. |
